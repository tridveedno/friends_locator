...
class ARLandmarkTracker:
    ...
    def __init__(self):
        ...
        self.orb = cv2.ORB_create(nfeatures=1000) # Reverted to 1000 features for ORB
    ...
    def match_features(self, descriptors1, descriptors2, use_sift=True):
        ...
        if use_sift:
            ...
            good_matches = sorted(matches, key=lambda x: x.distance)[:50] # Reverted to 50 ORB matches
    ...
    def analyze_ar_frame(self, current_frame_base64):
        """Analyze current camera frame for AR tracking"""
        if self.reference_descriptors is None:
            return {
                'success': False,
        ...
        # Prioritize SIFT for larger landmarks, no fallback to ORB
        current_keypoints, current_descriptors = self.extract_features(
            current_image, use_sift=True
        )
        
        if current_descriptors is None or len(current_keypoints) < 50: # Reverted to higher SIFT feature threshold
            return {
                'success': False,
        ...
        matches = self.match_features(
            ...
            current_descriptors, 
            use_sift=True # Always use SIFT matching
        )
        
        if len(matches) < 10: # Reverted to higher match threshold
            return {
                'success': False,
...
class ARNavigationSystem:
    ...
    def process_ar_frame(self, current_frame_base64):
        """Process current AR frame and return navigation instructions"""
        try:
            # Analyze current frame
            result = self.tracker.analyze_ar_frame(current_frame_base64)
            
            if not result['success']:
                # If tracking is lost or insufficient, return a 'poor' quality and no guidance
                return {
                    'success': False,
            ...
            return {
                ...
                'confidence': min(result.get('matches_count', 0) / 20.0, 1.0), # Max confidence at 20 matches
