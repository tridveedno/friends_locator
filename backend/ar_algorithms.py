...
class ARLandmarkTracker:
    ...
    def draw_navigation_arrow(self, image, rvec, tvec, distance, angle, direction):
        ...
        # Define arrow 3D points (larger arrow for visibility)
        arrow_length = 1.0  # Increased from 0.5m
        arrow_points = np.float32([
            [0, 0, 0],           # Arrow base
            [0, 0, arrow_length], # Arrow tip
            [-0.2, 0, arrow_length * 0.8],  # Left wing (larger)
            [0.2, 0, arrow_length * 0.8]   # Right wing (larger)
        ])
        ...
        image = cv2.arrowedLine(
            ...
            tipLength=0.3         # Larger tip
        )

        # Draw arrow wings
        image = cv2.line(image, tuple(img_points[1]), tuple(img_points[2]), (0, 255, 0), 3)
        image = cv2.line(image, tuple(img_points[1]), tuple(img_points[3]), (0, 255, 0), 3)

        # Add distance text with background
        text = f"{int(distance)}m {direction}"
    ...
    def analyze_ar_frame(self, current_frame_base64):
        ...
        # Extract features from current frame (try SIFT first, fallback to ORB)
        current_keypoints, current_descriptors = self.extract_features(current_image, use_sift=True)
        
        if current_descriptors is None or len(current_keypoints) < 50:
            logger.info("SIFT failed, falling back to ORB")
            current_keypoints, current_descriptors = self.extract_features(current_image, use_sift=False)
            use_sift = False
            if current_descriptors is None or len(current_keypoints) < 50:
                return {
                    'success': False,
                    ...
                    'tracking_quality': 'poor',
                    'output_image': image_to_base64(current_image)
                }
        
        # Match features
        matches = self.match_features(self.reference_descriptors, current_descriptors, use_sift=use_sift)
        ...
        homography, mask = self.calculate_homography(self.reference_keypoints, current_keypoints, matches)
        ...
        # Estimate pose
        rvec, tvec, corners = self.estimate_pose_from_homography(homography, self.reference_image.shape)
        ...
        output_image = self.draw_navigation_arrow(current_image, rvec, tvec, distance_meters, abs(angle), direction)
        
        # Determine tracking quality
        tracking_quality = 'good' if len(matches) > 20 else 'fair' if len(matches) > 8 else 'poor'
        ...
        return {
            ...
            'output_image': image_to_base64(output_image)
        }
